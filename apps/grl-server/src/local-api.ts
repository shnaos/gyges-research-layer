 ...
 *
 * The evaluate endpoint runs the real Capability Firewall. It performs NO web
 * transport, fetch, browser, or network egress — it returns the firewall
 * decision only. A firewall deny is a successful evaluation (HTTP 200,
 * `allowed: false`), never an HTTP error.
 */
 ...
 *
 * Reads `GRL_HOST`, `GRL_PORT`, and `GRL_MAX_BODY_BYTES`. The `env` argument is
 * injectable so configuration can be tested without mutating global state.
 */
